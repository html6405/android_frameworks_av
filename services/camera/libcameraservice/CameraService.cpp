...
void CameraService::onDeviceStatusChanged(const String8& id,
    ...
    if (state == nullptr) {
        ALOGE("%s: Physical camera id %s status change on a non-present ID %s",
                __FUNCTION__, id.c_str(), physicalId.c_str());
        return;
    }
...
int CameraService::getDeviceVersion(const String8& cameraId, int* facing, int* orientation) {
    ...
    if (facing) {
        res = mCameraProviderManager->getCameraInfo(cameraId.c_str(), &info);
        if (res != OK) return -1;
        *facing = info.facing;
        if (orientation) {
        ...
        if (deviceVersion > CAMERA_DEVICE_API_VERSION_1_0 &&
            ...
            *client = new CameraClient(cameraService, tmp, packageName, featureId,
            ...
            return STATUS_ERROR_FMT(ERROR_ILLEGAL_ARGUMENT,
                    ...
                    cameraId.c_str(), deviceVersion, halVersion);
        }
    }
    return Status::ok();
}
...
Status CameraService::connectHelper(const sp<CALLBACK>& cameraCb, const String8& cameraId,
    ...
    ALOGI("CameraService::connect call (PID %d \"%s\", camera ID %s) for HAL version %s and "
            "Camera API version %d", clientPid, clientName8.c_str(), cameraId.c_str(),
        ...
        case CAMERA_DEVICE_API_VERSION_3_1:
            if (apiVersion == API_VERSION_2) {
                ALOGV("%s: Camera id %s uses HAL version %d <3.2, doesn't support api2 without shim",
                        __FUNCTION__, id.c_str(), deviceVersion);
                *isSupported = false;
...
void CameraService::BasicClient::opChanged(int32_t op, const String16&) {
    ...
    if (res == AppOpsManager::MODE_ERRORED) {
        ...
        // error.
        if (!mUidIsTrusted) {
            if (isUidActive && supportsCameraMute()) {
                setCameraMute(true);
            } else if (!isUidActive
                || (!supportsCameraMute())) {
                block();
...
void CameraService::UidPolicy::onUidProcAdjChanged(uid_t uid, int32_t adj) {
                        ALOGV("%s: uid %d has oom_adj %d", __FUNCTION__, uid, adj);
}
